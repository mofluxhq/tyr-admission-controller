...
try {
  ...
  assert.equal(idleHeartbeat().demand[0].recentAdmissions, 0);
  assert.equal(idleHeartbeat().demand[0].recentRejections, 0);
  assert.equal(idleHeartbeat().demand[0].inFlight, 0);
} finally {
  mode.stop();
}
