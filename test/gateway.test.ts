...
describe("admission-gateway", () => {
  ...
  it("keeps request headers bound to the exact Latchflo grant that admitted them", async () => {
    const gw = await startGateway({
      maxConcurrent: 1,
      initialRevision: 0,
    });
    const grantA = {
      source: "latchflo" as const,
      grantId: "grant-a",
      controllerEpoch: 31,
      revision: 1,
    ...
    try {
      ...
      expect(first.headers.get("x-korrx-grant-id")).toBeNull();
      expect(first.headers.get("x-korrx-controller-epoch")).toBeNull();
      ...
      expect(second.headers.get("x-korrx-grant-id")).toBeNull();
      expect(second.headers.get("x-korrx-controller-epoch")).toBeNull();
    } finally {
      gw.server.close();
    }
  });

  it("attaches the exact Latchflo grant to an observe-mode bypass", async () => {
    const gw = await startGateway({
      maxConcurrent: 10,
      initialRevision: 0,
      budget: 0,
    ...
    try {
      ...
      const res = await fetch(`${gw.url}/v1/messages`, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify(msg("observe under Latchflo grant")),
      });
      ...
      expect(res.headers.get("x-korrx-grant-id")).toBeNull();
      expect(res.headers.get("x-korrx-controller-epoch")).toBeNull();
    } finally {
      gw.server.close();
    }
  });

  it("attaches the exact Latchflo grant to a rejected request", async () => {
    const gw = await startGateway({
      maxConcurrent: 1,
      initialRevision: 0,
    });
    ...
    try {
      ...
      const res = await fetch(`${gw.url}/v1/messages`, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify(msg("rejected under Latchflo grant")),
      });
  ...
  it("exports bounded Prometheus metrics and one completed admission audit", async () => {
    ...
    try {
      ...
      expect(metrics).toContain('tyr_build_info{version="0.32.0"} 1');
